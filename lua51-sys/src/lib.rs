...
extern "C" {
    ...
    pub fn luaL_openlib(L: *mut lua_State, libname: *const c_char, l: *const luaL_Reg, nup: c_int);
}
...
pub unsafe fn lua_tointegerx(L: *mut lua_State, idx: c_int, isnum: *mut c_int) -> lua_Integer{
    ...
    if isnum != ptr::null_mut() {
        *isnum = (n != 0 || lua_isnumber(L, idx) == 1) as c_int;
    }
    n
}
...
pub unsafe fn lua_tonumberx(L: *mut lua_State, idx: c_int, isnum: *mut c_int) -> lua_Number {
    ...
    if isnum != ptr::null_mut() {
        *isnum = (n != 0.0 || lua_isnumber(L, idx) == 1) as c_int;
    }
    n
}
